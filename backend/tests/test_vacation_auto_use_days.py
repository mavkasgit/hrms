"""Списание дней отпуска: только с не полностью закрытых периодов, от старых к новым."""
...
async def test_auto_use_days_debits_partially_closed_period_first(
    db_session,
    create_employee,
    create_order,
    create_vacation_period,
):
    """Частично закрытый период (остаток > 0) остаётся в очереди списания.

    Регрессия: раньше auto_use_days пропускал ЛЮБОЙ период с заданным
    remaining_days (в т.ч. частично закрытый), уводя дни в следующий период.
    Теперь пропускается только полностью закрытый (remaining_days == 0).
    """
    ...
    # Частично закрываем первый период: остаток 21 (создаётся partial_close транзакция).
    await vacation_period_service.partial_close_period(db_session, p1.id, remaining_days=21)
    await db_session.refresh(p1)

    # Отпуск 27 дней: 21 с частично закрытого (доедает до 0), 6 со следующего.
    await auto_use_days(
        db_session,
        employee.id,
        days_to_use=27,
    ...
    # Частично закрытый период списан первым и доеден до нуля (стал полностью закрытым).
    assert p1.used_days == 24
    assert p1.remaining_days == 0
    assert p2.used_days == 6


async def test_auto_use_days_skips_fully_closed_period_and_debits_next_open(
    db_session,
    create_employee,
    create_order,
    create_vacation_period,
):
    """Полностью закрытый период (remaining_days == 0) пропускается, списание уходит дальше."""
    employee = await create_employee(hire_date=date(2024, 1, 15))
    ...
    await vacation_period_service.close_period(db_session, closed.id)
    ...
    assert closed.used_days == 24  # закрыт через close_period, автосписание не добавило дней
