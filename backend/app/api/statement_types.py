...
async def list_statement_types(
    ...
    return await statement_type_service.get_statement_types(db, active_only=False)
...
async def get_template_variables(
    ...
    return {"variables": get_all_template_variables("statement")}
...
async def create_statement_type(
    ...
    return await statement_type_service.create_statement_type(db, data)


@router.put("/{statement_type_id}", response_model=dict[str, Any])
async def update_statement_type(
    ...
    return await statement_type_service.update_statement_type(db, statement_type_id, data)


@router.delete("/{statement_type_id}")
async def delete_statement_type(
    statement_type_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: str = Depends(_get_current_user_stub),
):
    await statement_type_service.delete_statement_type(db, statement_type_id)
    return {"message": "Тип заявления удален"}


@router.post("/{statement_type_id}/template", response_model=dict[str, Any])
...
async def delete_template(
    ...
    await statement_type_service.delete_template(db, statement_type_id)
    return {"message": "Шаблон удален"}


@router.get("/{statement_type_id}/template")
async def download_template(
    ...
    try:
        ...
        raise HTTPException(404, "Тип заявления не найден")
    if not stmt_type.template_filename:
        raise HTTPException(404, "Шаблон не найден")
    file_path = get_template_path(stmt_type)
...
async def statement_type_onlyoffice_config(
    statement_type_id: int,
    request: Request,
    ...
    config = onlyoffice_service.build_config(
        doc_type="statement_template",
        doc_id=statement_type_id,
        file_path=file_path,
...
@router.get("/{statement_type_id}/onlyoffice/file")
async def statement_type_onlyoffice_file(
    statement_type_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: str = Depends(_get_current_user_stub),
):
...
@router.post("/{statement_type_id}/onlyoffice/callback")
async def statement_type_onlyoffice_callback(
    statement_type_id: int,
    request: Request,
    db: AsyncSession = Depends(get_db),
    current_user: str = Depends(_get_current_user_stub),
...
@router.post("/{statement_type_id}/onlyoffice/forcesave")
async def statement_type_onlyoffice_forcesave(
    statement_type_id: int,
    data: OnlyOfficeForceSaveRequest,
    current_user: str = Depends(_get_current_user_stub),
):
    _ensure_onlyoffice_enabled()
    if not data.document_key.startswith(f"statement_template-{statement_type_id}-"):
