...
async def get_authentik_user_by_username(username: str) -> dict[str, Any] | None:
    """Find user in Authentik by username. Returns user dict or None if not found."""
    data = await _request("GET", "/core/users/", params={"username": username, "page_size": 5})
...
async def create_authentik_user(
    *,
    ...
    """
    Create user in Authentik (POST /api/v3/core/users/).
    Optionally adds user to initial groups (e.g. hrms-viewer).
    """
    ...
    result = await _request("POST", "/core/users/", json_body=payload)
    if groups:
        uuid_map = await _group_uuid_cache()
        for group_name in groups:
            group_uuid = uuid_map.get(group_name)
            if group_uuid:
                await _request(
                    "POST",
                    f"/core/groups/{group_uuid}/add_user/",
                    json_body={"pk": result["pk"]},
                )
    return result


async def create_authentik_invite_link(
    user_pk: int,
    *,
    enrollment_flow_slug: str | None = None,
    expires_hours: int = 72,
) -> str | None:
    """
    Create invitation in Authentik for enrollment stage.
    Returns invite URL (e.g. http://<host>:9000/if/flow/invite/?itoken=<invite-uuid>) or None.
    """
    flow_slug = enrollment_flow_slug or settings.AUTHENTIK_ENROLLMENT_FLOW_SLUG or "invite"

    from datetime import datetime, timezone, timedelta
    expires = (datetime.now(timezone.utc) + timedelta(hours=expires_hours)).isoformat()

    try:
        invite_data = await _request(
            "POST",
            "/stages/invitation/invitations/",
            json_body={
                "name": f"hrms-invite-user-{user_pk}",
                "expires": expires,
                "single_use": True,
                "flow": flow_slug,
            },
        )
        invite_pk = invite_data.get("pk") or invite_data.get("invite_uuid") or invite_data.get("id")
        if not invite_pk:
            return None
        base = public_base_url() or ""
        return f"{base}/if/flow/{flow_slug}/?itoken={invite_pk}"
    except AuthentikAdminError:
        return None
