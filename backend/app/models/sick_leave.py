...
class SickLeave(Base):
    ...
    sick_leave_type: Mapped[str] = mapped_column(String(50), nullable=False)
    
    # Документы
    certificate_number: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)
    issued_by: Mapped[Optional[str]] = mapped_column(String(200), nullable=True)
    
    # Статус
    status: Mapped[SickLeaveStatus] = mapped_column(
        SQLEnum(SickLeaveStatus), 
    ...
    # Relationships
    employee: Mapped["Employee"] = relationship("Employee", back_populates="sick_leaves")
    creator: Mapped["User"] = relationship("User", foreign_keys=[created_by], lazy="joined")
    updater: Mapped["User"] = relationship("User", foreign_keys=[updated_by], lazy="joined")
