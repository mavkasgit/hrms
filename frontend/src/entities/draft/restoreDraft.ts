...
/**
 * «Восстановить» черновика из попапа/страницы черновиков: переводит на страницу
 * создания соответствующего документа и заполняет форму данными черновика.
 * Все виды (включая одиночный приказ) идут через единый ?fillDraftId —
 * страница сама тянет form-data и заполняет форму.
 */
export async function restoreDraftFromForm(
  draftId: string,
