    ...
    <Dialog open={open} onOpenChange={(v) => !busy && onOpenChange(v)}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Текущее значение: <b>{currentValue} дн.</b> Примените к диапазону или поправьте
            значение в каждом периоде вручную.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-md border border-muted/40 p-3 space-y-2">
            ...
            <div className="max-h-64 overflow-y-auto rounded-md border border-muted/30 divide-y divide-muted/20 text-sm">
              {periodsDesc.map((p) => {
                ...
                return (
                  <div key={p.period_id} className="flex items-center justify-between gap-3 px-2 py-1.5">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className="text-xs text-muted-foreground">доп.</span>
                      <Input
                        type="number"
                        min={0}
                        value={editVal ?? ""}
                        ...
                        className={`w-16 h-7 text-center ${isDirty ? "border-blue-500 ring-1 ring-blue-300" : ""} ${isInvalid ? "border-red-500" : ""}`}
                      />
                    </div>
                  </div>
                )
