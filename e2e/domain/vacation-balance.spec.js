...
import { expectBalanceInvariant, expectNonNegativeAvailable, expectPeriodInvariant, } from '../helpers/vacation-invariants';
const API_BASE = '';
...
test.describe('Vacation Balance API tests', () => {
    ...
    test.afterEach(async ({ request }) => {
        ...
        for (const order of orders) {
            if (!BALANCE_EMP_RE.test(order.employee_name || ''))
                continue;
        ...
        for (const vacation of vacations) {
            if (!BALANCE_EMP_RE.test(vacation.employee_name || ''))
                continue;
        ...
        for (const employee of employees) {
            if (!BALANCE_EMP_RE.test(employee.name || ''))
                continue;
        ...
        for (const position of positions) {
            if (!BALANCE_POS_RE.test(position.name || ''))
                continue;
        ...
        for (const department of departments) {
            if (!BALANCE_DEPT_RE.test(department.name || ''))
                continue;
    ...
    test('2) after vacation creation - balance decreases', async ({ request }) => {
        ...
        const periodYear1 = periodsAfter.find((p) => p.year_number === 1);
        ...
        const periodYear1Restored = periodsRestored.find((p) => p.year_number === 1);
    ...
    test('6) multiple years - total balance', async ({ request }) => {
        ...
        const totalFromPeriods = periods.reduce((sum, p) => sum + p.total_days, 0);
    ...
    test('9) balance recalculation on vacation deletion', async ({ request }) => {
        ...
        const periodWith = periodsWith.find((p) => p.year_number === 1);
        ...
        const periodDeleted = periodsDeleted.find((p) => p.year_number === 1);
