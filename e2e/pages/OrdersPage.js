...
export class OrdersPage {
    ...
    async waitForOrderListRefresh(trigger) {
        const refreshPromise = this.page
            .waitForResponse((resp) => resp.url().includes('/api/orders') && resp.request().method() === 'GET', { timeout: 4000 })
            .catch(() => null);
        await trigger();
    ...
    async clickCreate() {
        const createPromise = this.page
            .waitForResponse((resp) => resp.url().includes('/api/orders') && resp.request().method() === 'POST', { timeout: 7000 })
            .catch(() => null);
