...
export class PositionsPage {
    ...
    async waitForStructureRefresh(trigger) {
        const refreshPromise = this.page
            .waitForResponse((resp) => resp.url().includes('/api/structure') && resp.request().method() === 'GET', { timeout: 4000 })
            .catch(() => null);
        await trigger();
    ...
    async getEmployeeCountForPosition(name) {
        const node = await this.getPositionNode(name);
    ...
    // ПРОВЕРКИ
    // ============================================================================
    async expectPositionVisible(name) {
        await expect(this.getPositionNode(name)).toBeVisible({ timeout: 5000 });
    }
    async expectPositionNotVisible(name) {
