...
export class EmployeesPage {
    ...
    async waitForEmployeesRefresh(trigger) {
        const refreshPromise = this.page
            .waitForResponse((resp) => resp.url().includes('/api/employees') && resp.request().method() === 'GET', { timeout: 4000 })
            .catch(() => null);
        await trigger();
    ...
    async getEmployeeTabNumber(name) {
    ...
    async createEmployeeViaUI(data) {
        await this.clickAdd();
    ...
    // ============================================================================
    async openEmployee(name) {
        const row = await this.getEmployeeRow(name);
        await row.click();
        await expect(this.dialog).toBeVisible({ timeout: 5000 });
    }
    async dismissEmployee(name) {
    ...
    async deleteEmployeePermanently(name) {
        await this.openEmployee(name);
