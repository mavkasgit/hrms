...
test.describe("Страницы отпуска за свой счет и вызовов в выходной", () => {
    ...
    test("рендерит страницу /unpaid-leaves и отправляет фильтр order_type_code", async ({ page }) => {
        const requestPromise = page.waitForRequest((request) => request.method() === "GET" &&
            request.url().includes("/api/orders/all") &&
            request.url().includes("order_type_code=vacation_unpaid"));
        await page.goto("/unpaid-leaves");
        await expect(page.getByRole("heading", { name: "Отпуск за свой счет", exact: true })).toBeVisible();
    ...
    test("ведет учет отпусков за свой счет и считает общее количество за выбранный период", async ({ page, apiOps }) => {
        const employee = await apiOps.createEmployee({ name: `E2E-Unpaid-Stat-${Date.now()}` });
        const unpaidTypeId = await apiOps.getOrderTypeId({ code: "vacation_unpaid" });
        await apiOps.createOrder(employee.id, {
            order_type_id: unpaidTypeId,
        ...
        await expect(page.getByTestId("unpaid-total-orders")).toHaveText("Всего отпусков за период: 2");
        ...
        await expect(page.getByTestId("unpaid-total-orders")).toHaveText("Всего отпусков за период: 1");
