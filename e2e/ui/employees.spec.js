...
import { makeEmployeeData, fillEmployeeForm, uid, comboboxCreate, dateField, fillGridInput, } from '../helpers/employee-helpers';
...
async function cleanupEmployeesArtifacts(request) {
    ...
    for (const employee of employees) {
        if (!EMPLOYEE_CLEANUP_RE.test(employee.name))
            continue;
    ...
    for (const position of positions) {
        if (!POSITION_CLEANUP_RE.test(position.name))
            continue;
    ...
    for (const department of departments) {
        if (!DEPARTMENT_CLEANUP_RE.test(department.name))
            continue;
